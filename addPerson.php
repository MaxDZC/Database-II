                ...
                <div class="animated fadeIn">
                    <div class="row">
                        <div class='col-sm-6 col-sm-offset-1'>
                            ...
                            <form name="myForm" method="POST" action="insertPerson.php?id=<?php echo $_GET['id']; ?>">
                                <select class="form-control" name="student_id" required>
                                    <option value = "" selected>Please select a student</option>
                                    <?php
                                        $result = mysqli_query($mysqli, "SELECT * FROM STUDENT");
                              //          $result = mysqli_query($mysqli, "SELECT * FROM EVENT_ATTENDANCE WHERE event_id = '".$_GET['id']."'");

            //                            $query = "SELECT * FROM STUDENT LEFT JOIN EVENT_ATTENDANCE ON EVENT_ATTENDANCE.student_id = STUDENT.student_id WHERE EVENT_ATTENDANCE.student_id IS NULL";
              //                          $result = mysqli_query($mysqli, $query);

                                        while($data = mysqli_fetch_array($result)) {
                                            $get = mysqli_query($mysqli, "SELECT * FROM EVENT_ATTENDANCE WHERE event_id = '".$_GET['id']."'");
